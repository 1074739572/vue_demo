  ...
  methods: {
    ...
    doDeleteRole(role){
      this.$confirm('此操作将永久删除[' + role.nameZh + ']角色, 是否继续?', '提示', {
        ...
        this.deleteRequest('/sys/basic/permiss/role/' + role.id).then(resp => {
          if (resp) {
            this.initRoles()
          }
        })
    ...
    doAddRole(){
      if (this.role.name && this.role.nameZh) {
        this.postRequest('/sys/basic/permiss/role',this.role).then(resp=>{
          if (resp) {
            this.initRoles()
            this.role.name = ''
            this.role.nameZh = ''
    ...
    // 获取所有角色
    initRoles() {
      this.getRequest('/sys/basic/permiss/getall').then(resp => {
        if (resp) {
          this.roles = resp
        }
      })
